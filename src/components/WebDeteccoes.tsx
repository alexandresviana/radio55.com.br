...
export default function WebDeteccoes() {
  ...
  const buscar = useCallback(
    async (opts?: { reescanear?: boolean }) => {
      ...
      const res = await fetch(`/api/web/deteccoes?${params}`);
  ...
  return (
    <section className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">Detecções na web</h2>
        </div>
        <button
          type="button"
      ...
      {loading && deteccoes.length === 0 ? (
        ...
        <p className="text-sm text-slate-400">
          ...
          portais e assuntos; as notícias já coletadas são reescaneadas automaticamente.
        </p>
      ) : (
        <>
                  ...
                  <tr key={item.id} className="border-b border-slate-50 align-top">
                    ...
                    <td className="px-2 py-3">
                      <div className="font-medium text-slate-800">
                        {item.fonte || item.dominio}
                      </div>
                      <div className="text-xs text-slate-400">{item.titulo}</div>
                    </td>
                    <td className="px-2 py-3 text-slate-600">
